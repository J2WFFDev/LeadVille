        ...
        try:
            ...
            if config_file.exists():
                try:
                    ...
                    if "timer" in config and "sensors" in config:
                        self.logger.info(f"✅ Loaded bridge config from file: timer={config['timer']}, sensors={len(config['sensors'])} device(s)")
                        for i, sensor in enumerate(config['sensors']):
                            self.logger.info(f"  Sensor {i+1}: {sensor}")
                        return config
                    else:
                        self.logger.warning("⚠️ Config file missing required keys, falling back to database/defaults")
                except Exception as e:
                    self.logger.warning(f"⚠️ Failed to read config file: {e}, falling back to database/defaults")
            else:
            ...
            if not db_path.exists():
                ...
                self.logger.warning("⚠️ Using hardcoded default configuration")
            ...
            # THIRD: Fall back to defaults (database query not working yet)
            default_config = {
                'timer': "60:09:C3:1F:DC:1A",  # Default AMG timer
                'sensors': ["EA:18:3D:6D:BA:E5", "C2:1B:DB:F0:55:50"]  # Default BT50 sensors
            }
            self.logger.warning("⚠️ Database query incomplete, using hardcoded default configuration")
            return default_config
            ...
            self.logger.warning("⚠️ Exception occurred, using hardcoded default configuration")
            return default_config
